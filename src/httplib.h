...
public:
  ...
  explicit Client2(const char *scheme_host_port,
                   const std::string &client_cert_path,
                   const std::string &client_key_path) {
    const static std::regex re(R"(^(https?)://([^:/?#]+)(?::(\d+))?)",std::regex_constants::icase);
...
inline std::string file_extension(const std::string &path) {
  std::smatch m;
  static auto re = std::regex("\\.([a-zA-Z0-9]+)$",std::regex_constants::icase);
...
inline bool parse_range_header(const std::string &s, Ranges &ranges) {
  static auto re_first_range = std::regex(R"(bytes=(\d*-\d*(?:,\s*\d*-\d*)*))",std::regex_constants::icase);
  std::smatch m;
  if (std::regex_match(s, m, re_first_range)) {
    ...
    split(&s[pos], &s[pos + len], ',', [&](const char *b, const char *e) {
      ...
      static auto re_another_range = std::regex(R"(\s*(\d*)-(\d*))",std::regex_constants::icase);
      std::cmatch cm;
...
inline bool parse_www_authenticate(const Response &res,
  ...
  if (res.has_header(auth_key)) {
    static auto re = std::regex(R"~((?:(?:,\s*)?(.+?)=(?:"(.*?)"|([^,]*))))~",std::regex_constants::icase);
...
inline bool Client::redirect(const Request &req, Response &res) {
  ...
  const static std::regex re(
      R"(^(?:(https?):)?(?://([^:/?#]*)(?::(\d+))?)?([^?#]*(?:\?[^#]*)?)(?:#.*)?)",std::regex_constants::icase);
