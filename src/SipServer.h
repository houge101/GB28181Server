...
public:
    ServerInfo(const char *ua,const char *nonce, const char *ip, int port, int rtpPort,
			...
			printf("mUa:%s   mNonce:%s  mIp:%s   mPort:%d   mrtpPort:%d   mSipId:%s   mSipRealm:%s   mSipPass:%s   mSipTimeout:%d   mSipExpiry:%d\r\n", \
            getUa(), getNonce(), getIp(), getPort(), getRtpPort(), \
            getSipId(), getSipRealm(), getSipPass(), getTimeout(), getExpiry());
			}
    ~ServerInfo() = default;
public:
