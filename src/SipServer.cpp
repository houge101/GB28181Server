...
int SipServer::request_invite(const char *device, const char *userIp, int userPort) {
    ...
    LOGI("getSipId : %s, getIp : %s, getPort %d, device : %s,  userIp : %s, userPort : %d\r\n", mInfo->getSipId(), mInfo->getIp(), mInfo->getPort(), device, userIp, userPort);

    sprintf(to, "sip:%s@%s:%d", mInfo->getNonce(), userIp, userPort);
    sprintf(from, "sip:%s@%s", mInfo->getSipId(),mInfo->getIp());
    //sprintf(contact, "sip:%s@%s:%d", mInfo->getNonce(),mInfo->getIp(), mInfo->getPort());
    snprintf(subject, 128, "%s:0,%s:0", mInfo->getNonce(), mInfo->getSipId());
    int ret = eXosip_call_build_initial_invite(mSipCtx, &msg, to, from, nullptr, subject);
    ...
    snprintf (sdp, 2048,
              ...
              "a=recvonly\r\n"
              "y=0100000001\r\n",
              mInfo->getNonce(),mInfo->getIp(), mInfo->getIp(), mInfo->getRtpPort());
