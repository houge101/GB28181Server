...
int Device::bind() {
    ...
    bind_address.sin_port = htons(10000);
    bind_address.sin_addr.s_addr = inet_addr(local_ip.c_str());

	printf("localPort %d,localIP : %s\r\n serverPort : %d, serverIP : %s\r\n",local_port, local_ip.c_str(), server_port, server_ip.c_str()); 
	...
	printf("rtpPort : %d,rtpIP : %s\r\n ",rtp_port, rtp_ip.c_str());
